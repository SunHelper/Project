...
static UWidgetBlueprint* GetOrCreateWidget(const FString& packageName, const FString& name, EObjectFlags Flags)
{
	...
	UPackage* widgetPackage = CreatePackage(*packageName);
...
static std::shared_ptr<LayerCollection> ParseLayers(PSDParser::Document* document, int32_t& currentIndex)
{
	...
	for (; currentIndex < document->GetLayerCount();)
	{
		...
		if (layer->IsFolderBegin())
		{
			...
			collection->Opacity = layer->GetOpacity() / 255.0f;
	...
	PSDParser::Rect area;
	area.Right = std::numeric_limits<int>::min();
	area.Left = std::numeric_limits<int>::max();
	area.Top = std::numeric_limits<int>::max();
	area.Bottom = std::numeric_limits<int>::min();
...
struct CreateWidgetsState
{
	int32_t emptyNameCount = 0;
	std::set<FString> usedNames;
};

static void CreateWidgets(
	UCanvasPanel* parentWidget,
	...
	CreateWidgetsState& state)
{
	...
	for (int32_t ind = (int32_t)layers->Children.size() - 1; ind >= 0; ind--)
	{
		...
		if (name == FString(TEXT("")))
		{
			name = FString(TEXT("PSD2UMG_Empty_")) + FString::FromInt(state.emptyNameCount);

			state.emptyNameCount++;
		}

		const auto getUniqueName = [&](const FString& name)
		{
			FString widgetName = ObjectTools::SanitizeObjectName(name);
			if (layer == nullptr)
			{
			}
		...
		if (state.usedNames.find(getUniqueName(name)) != state.usedNames.end())
		{
			int dupLoop = 0;
			FString tempName;
			do
			{
				tempName = name + FString(TEXT("_Dup_")) + FString::FromInt(dupLoop);
				dupLoop++;
			} while (state.usedNames.find(getUniqueName(tempName)) != state.usedNames.end());
			name = tempName;
		}

		state.usedNames.insert(getUniqueName(name));

		FString widgetName = InName.ToString() + TEXT("_") + getUniqueName(name);
		...
		if (layer == nullptr)
		{
			...
			if (slot != nullptr)
			{
				...
				slot->SetAnchors(anc);
				slot->SetPosition(pos / dpi);
				slot->SetSize(FVector2D(layers->Children[ind]->Area.Right - layers->Children[ind]->Area.Left, layers->Children[ind]->Area.Bottom - layers->Children[ind]->Area.Top) / dpi);
			}

			panel->RenderOpacity = layers->Children[ind]->Opacity;
		...
		else if (layer->ObjectType == PSDParser::LayerObjectType::Image)
		{
			...
			if (slot != nullptr)
			{
				...
				slot->SetAnchors(anc);
				slot->SetPosition(pos / dpi);
				slot->SetSize(FVector2D(layer->GetRect().Right - layer->GetRect().Left, layer->GetRect().Bottom - layer->GetRect().Top) / dpi);
			}

			image->ColorAndOpacity.A = layer->GetOpacity() / 255.0f;
		}
		else if (layer->ObjectType == PSDParser::LayerObjectType::Button)
		{
			if (texture == nullptr) continue;
			...
			if (layer->AdditionalObjectType == PSDParser::LayerAdditionalObjectType::Normal)
			{
				button->ColorAndOpacity.A = layer->GetOpacity() / 255.0f;
				button->BackgroundColor.A = layer->GetOpacity() / 255.0f;
			}
		}
		else if (layer->ObjectType == PSDParser::LayerObjectType::ProgressBar)
		{
			...
			else
			{
				...
				if (progressBar == nullptr)
				{
					...
					parentWidget->AddChild(progressBar);
				}
			...
			if (layer->AdditionalObjectType == PSDParser::LayerAdditionalObjectType::Background)
			{
				progressBar->RenderOpacity = layer->GetOpacity() / 255.0f;
			}
		}
		else if (layer->ObjectType == PSDParser::LayerObjectType::Text)
		{
			...
			if (text == nullptr)
			{
				...
				parentWidget->AddChild(text);
			}

			if (sizeof(TCHAR) == 2)
			{
				auto str = FString((const TCHAR*)layer->GetText());
			...
			else
			{
				...
				for (int c = 0; 260; c++)
				{
					temp[c] = layer->GetText()[c];
					if (temp[c] == 0) break;
				}
				temp[259] = 0;

				auto str = FString(temp);
				text->Text = FText::FromString(str);
			}
			...
			auto color = text->ColorAndOpacity.GetColor(FWidgetStyle());
			color.A = layer->GetOpacity() / 255.0f;
			text->ColorAndOpacity = FSlateColor(color);
		}
	}
	...
	FFeedbackContext* Warn)
{
	...
	{
		PSDParser::Rect area;
		area.Bottom = document->GetHeight();
		area.Right = document->GetWidth();
		area.Top = 0;
		area.Left = 0;
		layers->Area = area;

		// assume 4k
		dpi = document->GetHeight() / 1080.0f;
	}
